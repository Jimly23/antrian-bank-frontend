  ...
  return (
    <>
        <div className='grid grid-cols-6'>
            <div className='col-span-1 sidebar h-screen bg-violet-900'>
                <h3 className='font-medium text-xl text-white p-5'>Manager</h3>
                <ul className='mt-3 font-medium'>
                    ...
                    <li onClick={()=>setIsOpen(2)} className='text-white hover:bg-slate-500 cursor-pointer py-3 px-4'>Daftar User</li>
                </ul>
            </div>
            <div className='col-span-5 bg-violet-50 p-5'>
                {isOpen == 1 && 
                    <>
                        <>
                            ...
                            <div className="grid grid-cols-3 gap-x-10 pb-10 pt-2">
                                <div className="rounded-lg bg-white border w-full h-[100px] flex items-center gap-x-3 p-5">
                                    <HiMiniUserGroup className="text-slate-500" size={50}/>
                                    <div className="font-medium">
                                        <h5 className="text-lg">Total Antrian</h5>
                                        <p>{allQueue.length}</p>
                                    </div>
                                </div>
                                <div className="rounded-lg bg-white border w-full h-[100px] flex items-center gap-x-3 p-5">
                                    <HiMiniUser className="text-slate-500" size={50}/>
                                    <div className="font-medium">
                                ...
                                <div className="rounded-lg bg-white border w-full h-[100px] flex items-center gap-x-3 p-5">
                                    <FaRegCheckCircle className="text-slate-500" size={40}/>
                                    <div className="font-medium">
                        ...
                        <>
                            <div className='flex items-center justify-between mb-3'>
                              ...
                              <button onClick={() => window.print()} className='px-4 py-1 text-sm rounded-full bg-violet-900 text-white'>Cetak</button>
                            </div>
                            <div className="ringkasan-antrian w-full bg-white rounded-xl p-5 shadow">
                                <div className="header grid grid-cols-5 text-center font-medium text-slate-700 border-b border-slate-500 pb-6">
                                    <h6>Nama</h6>
                                    <h6>Telepon</h6>
                                    <h6>No. Antri</h6>
                                    {/* <h6>Mulai Antri</h6> */}
                                ...
                                {allQueue.map((item, index) => (    
                                    <div key={index} className="grid text-center items-center text-sm grid-cols-5 font-medium text-slate-700 border-b border-slate-500 py-3">
                                        <h6>{item.username}</h6>
                                        <h6>{item.telepon}</h6>
                                        <h6>{item.queue}</h6>
                                        {/* <h6>{item.startQueue}</h6> */}
                                        <h6>{item.estimationQueue}</h6>
                                        <div className={`py-1 px-3 w-[120px] mx-auto rounded-md text-white ${item.status === 'Success' ? 'bg-green-500 cursor-no-drop' : 'bg-orange-500'} font-medium cursor-pointer`}>
                                            {item.status}
                                        </div>
                                    </div> 
                                ))}
                ...
                {isOpen === 2 && 
                    <>
                        ...
                        <div className="daftar-antrian w-full bg-white rounded-xl p-5 shadow text-center">
                            ...
                            <div>
                                {allUser.map((item,index) => (
                                    <div key={index} className="grid items-center text-sm grid-cols-6 font-medium text-slate-700 border-b border-slate-500 py-3">
                                        <h6>{index + 1}</h6>
