        ...
        <div className='grid grid-cols-6'>
            ...
            <div className='col-span-5 bg-violet-50 p-5'>
                            ...
                            <div className="daftar-antrian w-full bg-white rounded-xl p-5 shadow">
                                ...
                                {allQueue.map((item, index) => (    
                                    <div key={index} className="grid text-center items-center text-sm grid-cols-5 font-medium text-slate-700 border-b border-slate-500 py-3">
                                        ...
                                        <div onClick={()=>handleUpdateStatus(item._id)} className={`py-1 px-3 w-[120px] mx-auto rounded-md text-white ${item.status === 'Success' ? 'bg-green-500 cursor-no-drop' : 'bg-orange-500'} font-medium cursor-pointer`}>
                                            {item.status}
                                        </div>
                                    </div> 
                                ))}
                ...
                {isOpen === 2 && 
                    <> 
                        <>
                            <h3 className='font-medium text-xl'>Riwayat Antrian</h3>
                            <div className="grid grid-cols-3 gap-x-10 pb-10 pt-2">
                                <div className="rounded-lg bg-white border w-full h-[100px] flex items-center gap-x-3 p-5">
                                    <HiMiniUserGroup className="text-slate-500" size={50}/>
                                    <div className="font-medium">
                                ...
                                <div key={index}>
                                    {item.status === 'Success' && 
                                        <div key={index} className="grid items-center text-sm grid-cols-6 font-medium text-slate-700 border-b border-slate-500 py-3">
                                                ...
                                                <h6>{item.startQueue}</h6>
                                                <h6>{item.estimationQueue}</h6>
                                                <div onClick={()=>handleUpdateStatus(item._id)} className={`py-1 px-3 mx-auto w-[120px] rounded-md text-white ${item.status === 'Success' ? 'bg-green-500 cursor-no-drop' : 'bg-orange-500'} font-medium cursor-pointer`}>
                                                    {item.status}
                                                </div>
                                        </div> 
                                    }
